...
export default function Dates(): JSX.Element {
  ...
  const [typesDogSelected, setTypesDogSelected] = useState<PickerItemType | undefined>();
  const [dateAppointmentSelected, setDateAppointmentSelected] = useState<PickerItemType | undefined>();
  const [timeAppointmentSelected, setTimeAppointmentSelected] = useState<PickerItemType | undefined>();

  //const [listTypesDogs, setListTypesDogs] = useState<{ id: string; label: string; }[]>([]);
  const [listTypesDogs, setListTypesDogs] = useState<{ id: string; label: string; }[]>(getListTypesDogs());
  const [listDayAvailable, setListDayAvailable] = useState<{ id: string; label: string; }[]>(getListDateAppontment());
  const [listTimesAvailable, setListTimesAvailable] = useState<{ id: string; label: string; }[]>(getListTimeAppontment());
