...
export const postCategoryAPIAction = createAsyncThunk(
  POST_CATEGORY,
  async (data_category: any) => {
    try {
      // Realiza una solicitud POST a la ruta de inicio de sesión en tu servidor
...
export const putCategoryAPIAction = createAsyncThunk(
  PUT_CATEGORY,
  async (data_category: CategoryDataType) => {
    try {
      // Realiza una solicitud POST a la ruta de inicio de sesión en tu servidor
      const response = await axios.post(urlCategory, data_category);
